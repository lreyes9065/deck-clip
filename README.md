...
This builds, tests, and validates `release/DeckClip-0.6.3.zip`. Its relevant layout is:
...
1. Copy `release/DeckClip-0.6.3.zip` to the Deck's Downloads folder. Do not extract it.
2. In Gaming Mode, open the Quick Access menu (`…`) and Decky Loader.
3. Open Decky settings and enable **Developer Mode** if needed.
4. Open the Developer section, choose **Install Plugin from Zip**, and select `DeckClip-0.6.3.zip` from Downloads.
